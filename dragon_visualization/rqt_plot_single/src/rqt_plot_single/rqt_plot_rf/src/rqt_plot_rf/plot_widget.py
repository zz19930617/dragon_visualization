...
JOINT_DATA = ['hip_vel_motor' , 'hip_vel_encorder']
...
class PlotWidget(QWidget):
    ...
    def __init__(self):
        ...
        #ui
        rp = rospkg.RosPack()
        ui_file = os.path.join('/home/robot/catkin_ws/src/dragon_visualization/rqt_plot_single/src/rqt_plot_single/rqt_plot_rf', 'resource' , 'plot.ui')
        loadUi(ui_file , self)
        ...
        try:
            ...
            try:
                ...
                #self.buff_y.append(msg.data[0])
                    self.curve['hip_vel_motor']['buff_y'].append(msg.data[0])
                    self.curve['hip_vel_motor']['buff_x'].append(rospy.get_time() - self.start_time)
        ...
        try:
            ...
            try:
                if msg.position is not []:
                    #self.curve['hip_vel_encorder']['buff_y'].append(msg.position[0])
		    self.curve['hip_vel_encorder']['buff_y'].append(msg.velocity[0])
                    self.curve['hip_vel_encorder']['buff_x'].append(rospy.get_time() - self.start_time)
            except AttributeError as e:
