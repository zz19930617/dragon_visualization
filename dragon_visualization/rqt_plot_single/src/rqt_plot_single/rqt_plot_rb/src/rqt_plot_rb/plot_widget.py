...
JOINT_DATA = ['knee_vel_motor' , 'knee_vel_encorder']
...
class PlotWidget(QWidget):
    ...
    def __init__(self):
        ...
        #ui
        rp = rospkg.RosPack()
        ui_file = os.path.join('/home/robot/catkin_ws/src/dragon_visualization/rqt_plot_single/src/rqt_plot_single/rqt_plot_rb' , 'resource' , 'plot.ui')
        loadUi(ui_file , self)
        ...
        for i in range(len(self._joint_data)):
            self.curve[self._joint_data[i]] = {'topic_name':self._joint_data[i] , 'buff_x':[] , 'buff_y':[] , 'buff_x_temp':[] , 'buff_y_temp' : [] ,'enable' : True}
        ...
        try:
            ...
            try:
                ...
                #self.buff_y.append(msg.data[0])
                    self.curve['knee_vel_motor']['buff_y'].append(msg.data[3])
                    self.curve['knee_vel_motor']['buff_x'].append(rospy.get_time() - self.start_time)
        ...
        try:
            ...
            try:
                if msg.position is not []:
                    #self.curve['knee_vel_encorder']['buff_y'].append(msg.position[1])
		    self.curve['knee_vel_encorder']['buff_y'].append(msg.velocity[1])
                    self.curve['knee_vel_encorder']['buff_x'].append(rospy.get_time() - self.start_time)
            except AttributeError as e:
